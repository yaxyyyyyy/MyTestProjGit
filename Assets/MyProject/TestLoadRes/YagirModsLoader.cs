...
public class YagirModsLoader : MonoBehaviour
{
    ...
    /// </summary>
    public YagirModsLoaderChain loadChain { get; private set; } = new YagirModsLoaderChain();
    ...
    public void Init(string modsFolder, string modsChainFolder, YagirModAssemblyLoader loader)
    {
        var unpackPath = modsFolder + "/Unpacked/";
        modArchives = GetModsList(modsFolder);
        loadChain = YagirModsLoaderChain.LoadFile(modsChainFolder);
    ...
    /// <returns></returns>
    public List<Mod> GetAssetBundles(List<string> paths, YagirModAssemblyLoader assemblyLoader, YagirModsLoaderChain chain)
    {
    ...
    /// <param name="modsChainFolder"></param>
    /// <returns></returns>
    public YagirModsLoaderChain InitChain(List<string> mods, YagirModsLoaderChain chain, string modsChainFolder)
    {
        if (chain == null)
        {
            chain = new YagirModsLoaderChain
            {
                mods = mods
            };
        }
